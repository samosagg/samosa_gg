...
use crate::{
    cache::Cache, db_models::{tokens::Token, wallets::Wallet}, telegram_bot::{actions::CallbackQueryProcessor, commands::mint::build_text_for_wallet_not_created, TelegramBot}, utils::{database_connection::get_db_connection, view_requests::view_fa_balance_request}
};
...
impl CallbackQueryProcessor for Balances {
    async fn process(
        ...
        let primary_wallet_opt = Wallet::get_primary_wallet_by_user_id(self.user_id, &mut conn).await?;
