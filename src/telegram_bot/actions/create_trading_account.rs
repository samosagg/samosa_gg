// use crate::{
//     cache::Cache,
//     db::{ users::NewTelegramUser, wallets::NewWallet },
...
//         let wallet_name = format!("aptos-test-wallet-{}", telegram_id);
...
//         let create_wallet_query = diesel
//             ::insert_into(wallets::table)
//             .values(new_wallet)
//             .on_conflict_do_nothing();
...
//         let hash = cfg.aptos_client.submit_transaction_and_wait(signed_txn).await?;

//         tracing::info!("Delegate trading to hash: {}, sender({})", hash, wallet_address);

//         bot
//             .edit_message_text(chat_id, msg.id(), build_text_for_existing_user(&wallet_address))
