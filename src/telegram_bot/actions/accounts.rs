...
use crate::{
    cache::Cache, db_models::{tokens::Token, wallets::Wallet}, telegram_bot::{actions::CallbackQueryProcessor, build_text_for_contact_support, escape_markdown_v2, TelegramBot}, utils::{database_connection::get_db_connection, view_requests::view_fa_balance_request}
};
...
impl CallbackQueryProcessor for Accounts {
    async fn process(
        ...
        let pending_message = bot.send_message(msg.chat().id, "Fetching wallets...").await?;
        ...
        if !wallets.is_empty() {
            for (idx, wallet) in wallets.iter().enumerate() {
                ...
                text.push_str(&build_text_for_wallet(idx + 1, &wallet.address, wallet.is_primary, balance / 10u64.pow(db_token.decimals as u32), &db_token.symbol));
                text.push_str("\n\n");
            }
        }
...
fn build_text_for_wallet(idx: usize, address: &str, is_primary: bool, balance: u64, symbol: &str) -> String {
    format!(
        "{}. {} {}\n\n\
        Address: `{}`\n\
        Balance: {} {}\n\
