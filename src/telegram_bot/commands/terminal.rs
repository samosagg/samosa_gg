...
use crate::{
    cache::Cache, db_models::users::User, http_server::middlewares::authentication::TelegramClaims, telegram_bot::{commands::{mint::build_text_for_wallet_not_created, CommandProcessor}, TelegramBot}, utils::database_connection::get_db_connection
};
use anyhow::Context;
...
impl CommandProcessor for Terminal {
    async fn process(
        ...
        let (value_str, unit) = cfg.config.jwt_config.expires_in.as_ref().map(|e| e.split_at(e.len() - 1)).unwrap_or(("30", "d"));
        ...
        let token = match token_result {
            Ok(token) => token,
            Err(_) => {
                bot.send_message(msg.chat.id, "Failed to generate token").await?;
                return Ok(())
            }
        };
        let login_url = Url::parse(
            &format!("{}/tg-login?token={}", cfg.config.terminal_url, token)
        )?;
        let kb = InlineKeyboardMarkup::new(
            vec![
                vec![
                    InlineKeyboardButton::url("Login on terminal", login_url)
                ]
            ]
        );
